  ...
  const initPay =(order)=>{
    const options = {
      ...
      handler: async (response)=> {
        ...
        try{
          const {data}= await axios.post(backendUrl + '/api/user/verfiyRazorpay',response,{headers:{token}});
          if(data.success){
            getUserAppointment()
        ...
        {appointments.map((item,index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && item.payment && <button className='sm:min-w-48 py-2 border rounded text-white bg-green-400'>Paid</button>}
